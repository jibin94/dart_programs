...
void main() {
  ...
  for (int i in input) {
    ///In output map we need to store the key as a numbers from the list and value as a count of the each numbers
    ...
    /// third iteration
    /// i is 3;
    /// output[3] does not exist so (output[3]??0) is 0
    /// (output[3]??0)+1 is 1
    ...
    /// fourth integration
    /// i is 4
    /// output[4] is does not exist, so output[4]??0 is 0;
    /// (output[4]??0)+1 is 1
    /// so output[4] is 1
    ...
    /// seventh iteration
    /// i is 5
    /// output[5] is exist, so output[5]??0 returns 2
    /// (output[5]??0)+1 is 3
